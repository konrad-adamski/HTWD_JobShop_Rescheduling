...
def get_undone_operations_df(df_plan, df_exec):
    ...
    return df_result.sort_values(by="Start")
"""

def get_undone_operations_df(df_plan, df_exec):
    """
    Gibt alle Operationen aus df_plan zurück, die noch nicht in df_exec vorkommen,
    basierend auf den Spalten 'Job' und 'Operation'.
    """
    # Finde alle Job-Operation-Paare, die in df_plan aber nicht in df_exec sind
    df_diff = df_plan.merge(
        df_exec[["Job", "Operation"]],
        on=["Job", "Operation"],
        how="left",
        indicator=True
    ).query('_merge == "left_only"').drop(columns="_merge")

    # Sortieren nach geplantem Start
    return df_diff.sort_values(by=["Job", "Start"])
